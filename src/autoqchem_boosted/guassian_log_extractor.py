...
class gaussian_log_extractor(object):
    ...
    def __init__(self, smiles, log_file_path):
        ...
        def check_for_exceptions(self):
            ...
            try:
                ...
                print('No geometry exists', flush=True)
            ...
            try:
                ...
                if [*filter(lambda x: x < 0., freqs)]:  # check for negative frequencies
                    print('Negative frequencies are present', flush=True)
                    raise NegativeFrequencyException()
            except TypeError:  # no frequencies
                print('Opt incomplete', flush=True)
                raise OptimizationIncompleteException()
            try:
                # The number of "stationary point found" messages should be the same as the number of "Normal termination" messages for a successful run
                freq_stationary_state_flag = len(re.findall("Stationary point found.", self.parts['freq']))
                freq_normal_termination_flag = len(re.findall("Normal termination", self.parts['freq']))
                # print(freq_stationary_state_flag, freq_normal_termination_flag, flush=True)
                if freq_stationary_state_flag != freq_normal_termination_flag:
                    print('Freq did not converge properly', flush=True)
                    raise NonConvergedFrequencyException()
            except:
                print('something might be wrong with freq', flush=True)
                raise GeneralException()

        # search each part for a Normal Termination flag and throw an error if at least one is not present
        try:
            normal_termination_counts = 0
            opt_normal_termination = len(re.findall("Normal termination", self.parts['opt']))
            normal_termination_counts += opt_normal_termination
            freq_normal_termination = len(re.findall("Normal termination", self.parts['freq']))
            normal_termination_counts += freq_normal_termination
            td_normal_termination = len(re.findall("Normal termination", self.parts['td']))
            normal_termination_counts += td_normal_termination
            if normal_termination_counts < 3:
                raise GeneralException()
        except: 
            print('One step did not have a normal termination', flush=True)
            raise GeneralException()

        try:    
            error_term_flag = len(re.findall("Error termination", self.log))
            #print(error_term_flag, flush=True)
            if error_term_flag > 0:
                print('There is an error termination somewhere', flush=True)
                raise ErrorTerminationException()
        except:
            print('Cannot check for error termination flags', flush=True)
            raise GeneralException()
    ...
    def _split_parts(self) -> None:
        ...
        log_parts = re.split("\n\s-+\n\s#\s", self.log)[1:]
        self.parts = {}
        # # original code for all
        # for p in log_parts:
        #     # regex logic: find first word in the text
        #     name = re.search("^\w+", p).group(0).lower()
        #     self.parts[name] = p

        # After adding in the additional freq re-opt, need to use the following:
        # opt will always be the first block; freq will always be 2nd to last part; td will always be the last part of the log file
        self.parts['opt'] = log_parts[0]
        self.parts['freq'] = log_parts[len(log_parts)-2]
        self.parts['td'] = log_parts[len(log_parts)-1]
...
class NonConvergedFrequencyException(Exception):
    """Raised when the frequency optimization has not converged in the Gaussian log file. Try adding another opt=ReadFC freq line to the gjf file"""
    pass

class ErrorTerminationException(Exception):
    """Raised when "Error termination" is found in one part in the Gaussian log file. Manually check the log file to determine next steps"""
    pass

class GeneralException(Exception):
    """Raised when a nonspecific error is found. Manually check the log file to determine next steps"""
    pass
 

# converts log files to json files for molecules
def convert_logfiles_to_jsonfiles():
    smiles_id_df = pd.read_csv(f'{main_datasets_dir}/exp_transaminase_unique_molecules.csv')
    smiles_list, molecule_ids = smiles_id_df['SMILES'].to_list(), smiles_id_df['Molecule_ID'].to_list()
    count = 0

    for smi,molecule_id in zip(smiles_list,molecule_ids):
        molecule_file_path = f"{directory}/multiconf_output_files/{molecule_id}"
        # if directory for this molecule does not exist in molecule_descriptors folder, make it
        if not os.path.exists(f'{main_datasets_dir}/molecule_descriptors/{molecule_id}'):
            os.makedirs(f'{main_datasets_dir}/molecule_descriptors/{molecule_id}', exist_ok=True)

        for file_path in glob.glob(f'{molecule_file_path}/*.log'):
            conf_id = file_path.split('/')[-1].split('.gjf.log')[0]
            gle = gaussian_log_extractor(smi, file_path)
            if gle.n_tasks > 0: # if the gaussian job was successful
                desc = json.dumps(gle.get_descriptors())
                with open(f"{main_datasets_dir}/molecule_descriptors/{molecule_id}/{conf_id}.json", "w") as outfile:
                    outfile.write(desc)
                
                count += 1
    
    print(f'Successfully extracted descriptor json files for {count} Gaussian jobs')
    return